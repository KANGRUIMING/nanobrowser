...
import { LLMProviderEnum } from '@extension/storage';
        ...
        switch (message.type) {
          ...
          case 'follow_up_task': {
            ...
            if (currentExecutor) {
              ...
              // Create a new executor if none exists
              logger.info('follow_up_task: creating new executor');
...
async function setupExecutor(
  ...
  if (apiKey && modelName && providerName) {
    ...
    const navigatorLLM = createChatModel(
      ...
      providerName as LLMProviderEnum,
      providerConfig,
      modelName
    );
    
    // Use the same model for all agents to simplify
    const plannerLLM = createChatModel(
    ...
    const validatorLLM = createChatModel(
  ...
  // Fallback to original storage-based method
