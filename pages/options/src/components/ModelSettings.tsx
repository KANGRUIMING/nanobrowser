...
const HARDCODED_API_KEY = 'your-hardcoded-api-key'; // Replace with your actual API key
...
export const ModelSettings = ({ isDarkMode = false }: ModelSettingsProps) => {
  const [apiKeys, setApiKeys] = useState<Record<LLMProviderEnum, { apiKey: string; baseUrl?: string }>>({
    [LLMProviderEnum.ChatGPT]: { apiKey: HARDCODED_API_KEY },
  });
  ...
  useEffect(() => {
    // No need to load API keys from storage, as they are hardcoded
  }, []);
  ...
  const handleSave = async (provider: LLMProviderEnum) => {
    try {
      // Implementation of handleSave method
    } catch (error) {
  ...
  const handleDelete = async (provider: LLMProviderEnum) => {
    try {
      // Implementation of handleDelete method
    } catch (error) {
